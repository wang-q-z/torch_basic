...
VGG16 = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 512, 512, 512, 'M']#通道数
...
class VGG_net(nn.Module):
    ...
    def create_conv_layers(self,architectures):
        ...
        for x in architectures:
            if type(x) == int:           #卷积层
                layers += [
                    nn.Conv2d(in_channels=inchannels, out_channels=x, kernel_size=(3,3), stride=(1,1), padding=(1,1)),
                    nn.BatchNorm2d(x),
                    nn.ReLU()
                ]
                inchannels = x
